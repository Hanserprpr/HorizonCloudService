...
// PathUtils 路径工具函数 (local copy to avoid circular imports)
...
func (PathUtils) ValidateFileName(filename string) error {
	if filename == "" {
		return NewServiceError("InvalidFileName", "filename cannot be empty", nil)
	}
	
	if len(filename) > 255 {
		return NewServiceError("InvalidFileName", "filename too long", nil)
	}
	...
	for _, char := range invalidChars {
		if strings.Contains(filename, char) {
			return NewServiceError("InvalidFileName", 
				fmt.Sprintf("filename contains invalid character: %s", char), nil)
		}
	}
	...
	for _, reserved := range reservedNames {
		if strings.EqualFold(nameWithoutExt, reserved) {
			return NewServiceError("InvalidFileName", 
				fmt.Sprintf("filename is reserved: %s", reserved), nil)
		}
	}
